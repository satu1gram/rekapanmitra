...
# 📱 SUPLEMEN: PANDUAN DESAIN MOBILE-FIRST

> Tambahkan semua spesifikasi di bagian ini ke implementasi. 85–90% pengunjung landing page produk kesehatan Indonesia mengakses via smartphone. Halaman ini **harus terasa native di mobile**, bukan sekadar "bisa dibuka di HP".

---

## 📐 PRINSIP UTAMA MOBILE DESIGN

```
1. Thumb Zone First   — CTA utama selalu di 40% bawah layar
2. One-Column Truth   — Satu kolom untuk semua konten di bawah 640px
3. Touch-Friendly     — Semua tap target minimal 44×44px (Apple HIG standard)
4. Scroll, Don't Zoom — Semua teks terbaca tanpa pinch-zoom (min 14px)
5. Fast Feel          — Skeleton loaders, optimistic UI, no layout shift
6. Thumb-Reachable CTA — Tombol paling penting: bottom-center atau bottom-right
```

---

## 🧩 KOMPONEN MOBILE PER SECTION
...
```
Height: 56px (lebih compact dari desktop 64px)
Layout: justify-between items-center px-4

KIRI:
  Logo: "BP·Group" — DM Sans 18px bold, warna --color-green-dark

KANAN (flex gap-2):
  Tombol WA kecil:
    Style: bg-[#25D366] text-white text-xs px-3 py-1.5 rounded-full font-semibold
    Text: "WA" + ikon WA 14px
    (versi pendek dari desktop)
  
  Hamburger button:
    Style: w-10 h-10 flex flex-col justify-center items-center gap-1.5
           border border-green-100 rounded-xl bg-white
    3 garis: w-5 h-0.5 bg-[--color-text] rounded-full
    Animasi open: garis atas rotate 45deg, tengah fade, bawah -45deg

DRAWER MENU (slide-in dari kanan):
  Width: 75vw, max 280px
  Background: bg-white
  Shadow: shadow-2xl
  Padding: pt-20 px-6 pb-8
  
  Menu items (flex-col gap-1):
    Style: px-4 py-3 rounded-xl text-base font-medium color-text
    Hover/active: bg-[--color-green-ultra] text-[--color-green]
    Items: Konsultasi Kesehatan | Produk | Jadi Mitra | Tentang Kami
  
  Di bawah menu (mt-8):
    Tombol WA full-width:
      "💬 Chat WhatsApp Sekarang"
      Style: w-full bg-[#25D366] text-white py-3.5 rounded-xl font-semibold text-center
    
    Disclaimer kecil:
      "Respon dalam 5 menit · Gratis"
      Style: text-xs text-center text-muted mt-2

OVERLAY:
  Saat drawer open: bg-black/40 backdrop-blur-sm menutup konten di belakang
  Tap overlay = tutup drawer
```
...
```
Stack layout (flex-col), bukan side-by-side

HEADER (px-4 text-center):
  Badge label: sama, tapi font lebih kecil (12px)
  Headline: "Ceritakan Kondisimu." — clamp(1.8rem, 7vw, 2.5rem)
  Sub: font-size 14px, max-w-xs mx-auto

INPUT CARD (mx-4 rounded-2xl):
  
  Quick chips:
    Layout: flex flex-wrap gap-2
    Ukuran chip: py-2 px-3 text-sm (sedikit lebih besar dari desktop untuk touch)
    Min height per chip: 36px
    
    ⚠️ PENTING: Chips di mobile HARUS mudah ditap.
    Jangan < 36px height. Jarak antar chip minimal 6px.

  Textarea:
    rows={3} di mobile (hemat ruang)
    Font-size: 16px (WAJIB ≥16px untuk mencegah iOS auto-zoom saat focus!)
    
  Tombol Submit:
    Style: w-full py-4 rounded-xl text-base font-bold
           (lebih tinggi dari desktop untuk thumb-friendly)

HASIL AI CARD (mt-4 mx-4):
  Muncul di BAWAH input card saat di mobile
  rounded-2xl p-5
  
  Smooth scroll: setelah result muncul, auto-scroll ke card ini
    window.scrollTo({ top: resultCardRef.offsetTop - 80, behavior: 'smooth' })
  
  Tampilan result tetap sama strukturnya, hanya font sedikit lebih kecil:
    - Teks empati: 13px
    - Tips title: 12px semibold
    - Tips desc: 11px
    - Produk reko: compact view (nama + alasan 1 baris)
```
...
```
Layout: overflow-x-auto (horizontal scroll)
Scrollbar: hidden (scrollbar-hide utility)
-webkit-overflow-scrolling: touch
scroll-snap-type: x mandatory

Container dalam: flex gap-6 px-4 w-max

Setiap item: scroll-snap-align: start, flex-shrink-0
  Style: flex items-center gap-1.5 text-white text-xs whitespace-nowrap

Scroll indicator (visual hint bahwa ada lebih):
  Gradient fade di kanan: 
    after: absolute right-0 top-0 h-full w-12 bg-gradient-to-l 
           from-[--color-green-dark] to-transparent pointer-events-none
```

---

### 📌 BRAND HERO (Healthy Living) — Mobile

```
Layout: flex-col (bukan grid 2 kolom)

TEKS DULU (di atas):
  px-4 pt-12 pb-6 text-center
  Label, Headline, Body, Stats seperti desktop tapi centered
  
  Stats row: flex justify-center gap-8
  (3 stats tetap muat dalam 1 baris, ukuran font dikecilkan)

VISUAL INGREDIENT CARDS (di bawah):
  mx-4 mt-4 bg-[--color-green-ultra] rounded-2xl p-4
  
  Grid: grid-cols-2 gap-3 (2×2 tetap seperti desktop)
  Ukuran card lebih compact: p-3
  Emoji: 24px (lebih kecil)
  Nama: 11px semibold
  Desc: 10px muted
```
...
```
Grid: grid-cols-1 gap-4 (bukan 4 kolom)

Setiap step card: flex items-start gap-4 (horizontal layout dalam card)
  KIRI: Nomor besar "01" + emoji — w-14 flex-shrink-0
  KANAN: Judul + deskripsi

Connector arrow: HIDDEN di mobile (none)

Atau alternatif yang lebih bagus:
  Tampilkan sebagai vertical timeline:
    - Garis vertikal di kiri (2px, warna --color-green-pale)
    - Dot di setiap step (10px circle, bg-[--color-green])
    - Konten di kanan garis
```

---

### 📌 KATALOG PRODUK — Mobile

```
FILTER TABS (KRITIKAL untuk mobile):

  Layout: overflow-x-auto scroll-smooth
  Scrollbar: tersembunyi
  Padding: px-4 pb-2
  Container dalam: flex gap-2 w-max

  Style tab: text-sm py-2 px-4 rounded-full whitespace-nowrap
             min-height: 36px
  
  FADE HINT di ujung kanan (menandakan ada tab lagi):
    Gradient overlay kanan: bg-gradient-to-l from-[--color-cream] to-transparent
    width: 32px, pointer-events-none
  
  STICKY FILTER: Saat scroll melebihi filter tabs,
    filter menjadi sticky top-[56px] (di bawah navbar) 
    dengan shadow kecil dan backdrop blur
    z-index: 40

PRODUCT GRID:
  Mobile: grid-cols-1 (single column, full width kartu)
  
  Kartu produk di mobile:
    Layout HORIZONTAL (berbeda dari desktop yang vertikal):
    
    flex flex-row items-stretch (bukan flex-col)
    
    KIRI — Visual area: w-32 h-full min-h-[120px]
      Gradient background per kategori
      Emoji produk centered (48px)
      
    KANAN — Info area: flex-1 p-4
      Nama: 15px semibold
      Tagline: 12px italic muted (max 1 baris, truncate)
      2 benefit bullets: 11px (hanya 2, bukan 3)
      Harga: 20px bold color-green
      
    BAWAH — CTA (full width, di bawah flex row):
      border-t border-green-50 px-4 py-3
      Tombol: full-width "💬 Pesan via WA" py-2.5 text-sm
      Micro-copy: "Gratis konsultasi · < 5 menit"
  
  ⚠️ CATATAN PENTING:
  Layout horizontal kartu di mobile jauh lebih efisien:
  - User bisa scan banyak produk tanpa scroll terlalu jauh
  - Visual produk tetap terlihat
  - Info penting tetap terbaca
  - Tombol CTA mudah dijangkau

ALTERNATIF UNTUK GRID 2-KOLOM (jika diinginkan):
  Gunakan grid-cols-2 dengan kartu VERTIKAL (seperti desktop)
  tapi dengan font lebih kecil dan padding lebih compact:
    - Gambar: h-32
    - Nama: 13px
    - Tagline: 11px (1 baris)
    - Benefit: hanya 2 item, 10px
    - Harga: 18px
    - Tombol: "WA" saja (icon + teks singkat)
```

---

### 📌 SECTION MITRA — Mobile

```
Layout: flex-col text-center px-4

Headline: font-size clamp(2rem, 8vw, 3rem)

3 Benefit Cards: grid-cols-1 gap-4
  Setiap card: flex items-start gap-4 text-left
    Ikon: 32px emoji + bg-white/20 rounded-xl p-2 flex-shrink-0
    Teks: kanan

CTA Tombol: full-width w-full py-4

VISUAL TRICK untuk mobile:
  Tambahkan background texture subtle:
  Floating leaf illustrations (SVG, opacity 0.06) di pojok-pojok section
  Ini membuat section terasa lebih premium meski hanya text+card
```
...
```
Layout: SWIPEABLE CAROUSEL (bukan 3 kolom statis)

Implementasi dengan CSS Scroll Snap:
  ...
  Setiap card:
    min-width: calc(85vw)  ← hampir full width, hint ada card berikutnya
    scroll-snap-align: start
    flex-shrink-0
    
  Pagination dots di bawah carousel:
    3 dots: w-2 h-2 rounded-full
    Active: bg-[--color-green] w-4 (melebar)
    Inactive: bg-green-200
    Transition: all 300ms ease
    
    Update dots dengan IntersectionObserver atau scroll event
```
...
Headline: font-size clamp(1.8rem, 7vw, 3rem)
  Jika terlalu panjang di mobile, break menjadi:
  "Siap Hidup" / "Lebih Sehat?" (2 baris)

CTA Buttons: flex-col gap-3 (bukan flex-row)
  Keduanya full-width

Micro-copy: text-xs, max-w-xs mx-auto
```
...
Position: fixed bottom-6 right-4 z-50
  (sedikit lebih ke kiri dari desktop agar tidak tertutup browser chrome)

Button: w-14 h-14 bg-[#25D366] rounded-full shadow-xl
  + pulse animation (::before)

UPGRADE: Expandable WA button di mobile

Default state: hanya ikon WA (compact, tidak ganggu scrolling)

Setelah 10 detik ATAU saat user scroll ke atas (mau pergi):
  Expand menjadi pill:
  "💬 Chat Sekarang"
  Style: rounded-full px-5 py-3.5 flex items-center gap-2
         transition: all 400ms cubic-bezier(0.34, 1.56, 0.64, 1)
  
  Auto-collapse setelah 5 detik jika tidak diklik
  
  Implementasi:
    useEffect(() => {
      const timer = setTimeout(() => setExpanded(true), 10000);
...
## 🎨 VISUAL TRICKS KHUSUS MOBILE

### 1. Gradient Overlays untuk Depth
```css
/* Membuat halaman terasa lebih dimensi di layar kecil */
...
/* Konsultasi section background texture */
.konsultasi-section {
  background-image: 
    radial-gradient(circle at 10% 20%, rgba(82, 183, 136, 0.06) 0%, transparent 50%),
    radial-gradient(circle at 90% 80%, rgba(184, 134, 11, 0.04) 0%, transparent 50%);
}
```

### 2. Card Micro-Shadow System
```css
/* Berbeda dari desktop — mobile butuh shadow yang lebih subtle */
/* karena layar lebih kecil dan shadow mudah terlihat berlebihan */

--shadow-mobile-card:   0 1px 8px rgba(45, 106, 79, 0.06),
                        0 4px 16px rgba(45, 106, 79, 0.06);
--shadow-mobile-hover:  0 4px 16px rgba(45, 106, 79, 0.12),
                        0 8px 24px rgba(45, 106, 79, 0.08);

@media (max-width: 640px) {
  .card { box-shadow: var(--shadow-mobile-card); }
  .card:active { 
    box-shadow: var(--shadow-mobile-hover);
    transform: scale(0.98); /* press effect saat tap */
    transition: all 150ms ease;
  }
}
```

### 3. Typography Scaling yang Benar
```css
/* Fluid typography — tidak perlu banyak breakpoint manual */
...
/* Body text — JANGAN di bawah 14px untuk readability */
.body-text {
  font-size: clamp(0.875rem, 3.5vw, 1rem);
  line-height: 1.65;
}

/* Label kecil — min 11px */
.label-sm {
  font-size: clamp(0.7rem, 2.5vw, 0.8rem);
  letter-spacing: 0.05em;
}

/* ⚠️ WAJIB: Input dan Textarea min 16px di iOS */
/* untuk mencegah halaman auto-zoom saat fokus */
input, textarea, select {
  font-size: max(16px, 1rem) !important;
}
```

### 4. Scroll Experience yang Enak
```css
/* Smooth scrolling global */
html {
  scroll-behavior: smooth;
  /* Mencegah overscroll bounce yang mengganggu */
  overscroll-behavior-y: contain;
}

/* Momentum scrolling untuk horizontal scroll areas */
.scroll-x {
  overflow-x: auto;
  overflow-y: hidden;
  -webkit-overflow-scrolling: touch;
...
/* Snap scrolling untuk carousel testimoni */
.carousel-track {
  scroll-snap-type: x mandatory;
...
.carousel-item {
  scroll-snap-align: start;
  scroll-snap-stop: always; /* tidak skip item saat swipe cepat */
}
```

### 5. Focus States yang Ramah Mobile
```css
/* Form elements: visible focus ring yang tidak terasa mengganggu */
input:focus, textarea:focus {
  outline: none;
  border-color: var(--color-green);
  box-shadow: 0 0 0 3px rgba(61, 122, 79, 0.12);
  transition: all 200ms ease;
}

/* Tombol: active state terasa "ditekan" */
button:active {
  transform: scale(0.97);
  transition: transform 100ms ease;
}

/* Chip: spring animation saat select */
.chip:active {
...
## 📏 SPACING SYSTEM UNTUK MOBILE

```css
/* Section padding — lebih compact di mobile */
@media (max-width: 640px) {
  .section-padding {
    padding-top:    3rem;   /* desktop: 5rem */
    padding-bottom: 3rem;
    padding-left:   1rem;
    padding-right:  1rem;
  }
  ...
  .card-padding {
    padding: 1.25rem;   /* desktop: 1.5rem */
  }
  
  /* Gap antar cards */
  .card-gap {
    gap: 1rem;   /* desktop: 1.5rem */
  }
  
  /* Headline margin-bottom */
  .headline-mb {
    margin-bottom: 1.5rem;   /* desktop: 2.5rem */
  }
}
```
...
## ⚡ PERFORMA MOBILE YANG KRITIS

### Image & Asset Optimization
```javascript
// 1. Lazy load semua gambar di bawah fold
<img 
  loading="lazy"
  decoding="async"
  src="product.jpg"
  alt="British Propolis BP Group"
/>

// 2. SVG inline untuk ilustrasi (tidak perlu HTTP request)
// Jangan gunakan <img src="illustration.svg"> — gunakan inline SVG

// 3. Preload font untuk mencegah FOUT (Flash of Unstyled Text)
// Tambahkan di <head>:
<link rel="preload" href="cormorant-garamond.woff2" as="font" crossorigin>
<link rel="preload" href="dm-sans.woff2" as="font" crossorigin>
```

### Skeleton Loading
```jsx
// Tampilkan skeleton saat konten loading
// Ini membuat halaman terasa instan meski konten belum siap
...
```css
/* Reservasi space untuk gambar sebelum loaded */
.product-visual {
  aspect-ratio: 4/3;  /* atau height: 200px fixed */
  background: var(--placeholder-gradient);
}

/* Font display swap untuk web fonts */
@font-face {
  font-display: swap;  /* teks langsung muncul dengan fallback font */
}
```
...
### Device Testing (test di semua ini):
```
Kecil:    iPhone SE (375×667px)
Standar:  iPhone 14 (390×844px), Samsung Galaxy A-series (360px wide)
Besar:    iPhone 14 Pro Max (430px wide)
Tablet:   iPad Mini (768px), Samsung Galaxy Tab

Orientasi: Portrait DAN Landscape di semua device
```

### Skenario Test yang Harus Dijalankan:
```
[ ] Buka halaman, langsung scroll dari atas ke bawah — apakah flow terasa natural?
[ ] Tap semua chip keluhan — apakah mudah ditap? Ada feedback visual?
[ ] Isi textarea + tap Analisis — apakah keyboard tidak menutupi tombol?
[ ] Tunggu loading — apakah animasi loading muncul dan smooth?
[ ] Baca hasil konsultasi — apakah teks terbaca tanpa zoom?
[ ] Scroll ke katalog — apakah filter tab bisa di-scroll horizontal?
[ ] Tap kartu produk — apakah ada hover state yang responsif?
[ ] Tap tombol WA — apakah WhatsApp terbuka dengan pesan yang benar?
[ ] Buka hamburger menu — apakah drawer slide-in smooth?
[ ] Rotasi ke landscape — apakah layout tidak berantakan?
[ ] Test dengan koneksi 3G (throttle di DevTools) — apakah skeleton muncul?
[ ] Test dengan satu tangan (thumb only) — apakah semua CTA bisa dijangkau?
```

### Browser Testing:
```
[ ] Safari iOS (paling penting — 58% pengguna iPhone di Indonesia)
[ ] Chrome Android
...
## 🎯 QUICK REFERENCE: UKURAN YANG WAJIB DIINGAT

| Elemen | Desktop | Mobile | Alasan |
|--------|---------|--------|--------|
| Tap target minimal | 36px | **44px** | Apple HIG / WCAG 2.5.5 |
| Font body minimal | 14px | **16px** (input) | Mencegah iOS auto-zoom |
| Font body minimal | 14px | **14px** | Readability |
| Section padding | py-20 | **py-12** | Efisiensi ruang |
| Card padding | p-6 | **p-4** | Efisiensi ruang |
| CTA height | py-3 | **py-3.5 – py-4** | Thumb-friendly |
| Gap antar card | gap-6 | **gap-4** | Efisiensi ruang |
| Border radius card | 16px | **16px** | Tetap sama, terasa premium |
| Shadow | medium | **subtle** | Layar kecil, shadow mudah berat |

---

## 💡 TIPS TAMBAHAN DARI EXPERT

### Psikologi Warna di Layar Kecil
```
Di mobile, warna terasa lebih intens karena layar lebih dekat ke mata.
Rekomendasi:
- Kurangi saturasi background 10–15% untuk versi mobile
- Pastikan contrast ratio teks ≥ 4.5:1 (WCAG AA)
- Gunakan bg-white untuk kartu, bukan off-white gelap
- --color-green-ultra (#F0F9F2) sebagai section background mobile sangat tepat
```

### Touch Gesture yang Natural
```jsx
// Swipe to dismiss untuk drawer menu
// Gunakan library react-swipeable atau implementasi manual:
...
// Swipe left/right untuk carousel testimoni
// CSS scroll snap sudah cukup — tidak perlu JavaScript tambahan
```

### "Safe Area" untuk iPhone dengan Notch/Dynamic Island
```css
/* Mencegah konten tertutup notch atau home indicator */
.sticky-nav {
...
*Panduan mobile ini adalah suplemen dari dokumen utama PROMPT_ANTIGRAVITY_KATALOG_BPGROUP.*  
*Implementasikan bersamaan dengan spesifikasi desktop di atas — bukan sebagai afterthought.*  
*Mobile-first bukan berarti desktop dikesampingkan — artinya mobile di-design dengan penuh perhatian yang sama.*
