        ...
        <div className="katalog-page-wrapper">
            ...
            <div className="ai-hero" id="konsultasi">
                ...
                <div className="ai-hero-right">
                    ...
                    {/*  Result  */}
                    {aiResult && (
                        <div className="result-content show" id="resultContent" style={{ maxHeight: '600px', overflowY: 'auto', scrollBehavior: 'smooth' }}>
                            {/* 1. BLOK EMPATI */}
                            <div style={{ background: 'var(--green-ultra)', borderRadius: '12px', padding: '16px', marginBottom: '16px', display: 'flex', gap: '12px', alignItems: 'flex-start' }}>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '20px' }}>
                                {aiResult.tips_gaya_hidup?.map((tip, idx) => (
                                    <div key={idx} style={{ background: '#F9FFF9', border: '1px solid var(--green-100)', borderRadius: '12px', padding: '12px', display: 'flex', alignItems: 'flex-start', gap: '12px' }}>
                                        <div style={{ fontSize: '24px' }}>{tip.icon || '✨'}</div>
                                        <div>
                                            <div style={{ fontSize: '13px', fontWeight: 600, color: 'var(--text)', marginBottom: '4px' }}>{tip.title}</div>
                                            <div style={{ fontSize: '12px', color: 'var(--text-muted)' }}>{tip.description}</div>
                                        </div>
                                    </div>
                                ))}
                            </div>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '20px' }}>
                                {aiResult.rekomendasi?.map((prod, idx) => (
                                    <div key={idx} style={{ border: '1px solid var(--border)', borderRadius: '12px', padding: '12px', display: 'flex', alignItems: 'center', gap: '12px' }}>
                                        <div style={{ width: '40px', height: '40px', background: 'var(--green-pale)', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '24px', flexShrink: 0 }}>
                                            {prod.emoji || '🌿'}
                                        </div>
                                        <div style={{ flex: 1 }}>
                                            <div style={{ fontSize: '13px', fontWeight: 600, color: 'var(--text)' }}>{prod.name}</div>
                                            <div style={{ fontSize: '11px', color: 'var(--text-muted)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: '180px' }}>{prod.reason}</div>
                                            <div style={{ fontSize: '12px', fontWeight: 600, color: 'var(--green)', marginTop: '2px' }}>{prod.price}</div>
                                        </div>
                                        <a href="#produk" style={{ fontSize: '11px', fontWeight: 600, color: 'var(--green)', border: '1px solid var(--green)', padding: '4px 8px', borderRadius: '6px', whiteSpace: 'nowrap' }}>
                                            Lihat →
                                        </a>
                                    </div>
                                ))}
                            ...
                            <a href={getOrderLink(aiResult.cta || "Halo, saya ingin konsultasi kesehatan lebih lanjut dengan BP Group")} target="_blank" rel="noreferrer" style={{ display: 'block', width: '100%', background: '#25D366', color: 'white', padding: '12px', borderRadius: '12px', fontWeight: 600, textAlign: 'center', textDecoration: 'none', marginBottom: '4px' }}>
                                💬 {aiResult.cta || "Konsultasi Lebih Lanjut via WhatsApp"}
                            </a>
                            <div style={{ textAlign: 'center', fontSize: '11px', color: 'var(--text-muted)', marginBottom: '20px' }}>
                                Gratis · Tidak ada tekanan · Respon &lt; 5 menit
                            </div>
                    ...
                    <div className="product-grid" id="productGrid">
                        {KATALOG_PRODUCTS.map(product => (
                            <div key={product.id} className={`product-card fade-in ${activeCategory === 'all' || activeCategory === product.category ? 'visible' : 'hidden'}`}>
                                <div className="card-img" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: 'var(--green-ultra)' }}>
                                    <div className={`card-img-bg ${product.bgStyle}`}></div>
                                    <span style={{ fontSize: '80px', position: 'relative', zIndex: 2, filter: 'drop-shadow(0 10px 15px rgba(0,0,0,0.15))' }}>{product.emoji}</span>
                                </div>
                                <div className="card-body">
                                    ...
                                    <div className="card-specs">
                                        {product.specs.map((spec, idx) => (
                                            <span key={idx} className="spec-tag">{spec.icon} {spec.text}</span>
                                        ))}
                                    </div>
                                    <div className="card-footer" style={{ flexDirection: 'column', gap: '12px', alignItems: 'stretch' }}>
                                        ...
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                                            {storeSlug ? (
                                                ...
                                                <a href={getOrderLink(`Halo kak, saya mau konsultasi & pesan ${product.name}.`)} className="btn-cta-katalog" target="_blank" rel="noreferrer">
                                                    💬 Konsultasi & Pesan via WA
                                                </a>
                                            )}
                ...
                <div className="container">
                    <div className="mitra-header fade-in">
                        ...
                        <p style={{ color: 'rgba(255,255,255,0.7)', fontSize: '16px', maxWidth: '600px', margin: '0 auto' }}>
                            Bergabunglah bersama ribuan mitra BP Group Quantum Millionaire yang telah membuktikan kesuksesan finansial mereka.
                        </p>
                    </div>

                    <div className="mitra-grid">
                        <div className="mitra-card fade-in">
                            <div className="mitra-icon">💰</div>
                        ...
                        <div className="mitra-card fade-in" style={{ animationDelay: '0.1s' }}>
                            <div className="mitra-icon">☪️</div>
