-- ============================================================
-- ADD STOCK VALIDATION & DEDUCTION TO submit_public_order
--
-- Validasi stok sebelum order publik diproses:
--   1. Cek apakah user_stock.current_stock >= total qty
--   2. Jika tidak cukup → return error
--   3. Jika cukup → kurangi stok setelah order berhasil
-- ============================================================
...
BEGIN
  ...
  -- ═══════════════════════════════════════════════════════════
  -- 3. CEK STOK — validasi sebelum lanjut
  -- ═══════════════════════════════════════════════════════════
  SELECT COALESCE(current_stock, 0) INTO v_current_stock
  FROM public.user_stock
  WHERE user_id = v_user_id;

  IF v_current_stock < v_total_qty THEN
    RETURN jsonb_build_object(
      'success', false,
      'error', format('Stok tidak mencukupi. Butuh %s botol, tersedia %s botol.', v_total_qty, v_current_stock)
    );
  END IF;

  -- 4. Tier, buy_price, margin
  ...
  -- 7. UPSERT CUSTOMER → dapatkan customer_id
  BEGIN
  ...
  -- ═══════════════════════════════════════════════════════════
  -- 10. STOCK DEDUCTION — kurangi stok setelah order berhasil
  -- ═══════════════════════════════════════════════════════════
  BEGIN
    ...
    END IF;
  EXCEPTION WHEN OTHERS THEN
    RAISE WARNING 'Stock deduction failed after order %: %', v_order_id, SQLERRM;
  END;
